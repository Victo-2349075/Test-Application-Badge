...
{
    $request->authenticate(); // vérifie le rate limiting
    $credentials = request(['email', 'password']);
    if (!Auth::attempt($credentials)) {
        RateLimiter::hit($request->throttleKey(), 60); // compte les échecs
        return response()->json([
            'message' => 'Unauthorized'
        ], 401);
    }
     RateLimiter::clear($request->throttleKey()); // reset si succès
    ...
    public function logout(Request $request)
{
    ...
    return response()->json([
        ...
        'access_token',
        '',
        -1,      // expiration dans le passé = suppression
        '/',
        null,
        true,
        true,
    ...
    private function createToken(User $user): PersonalAccessTokenResult
{
    ...
    $token->token->expires_at = Carbon::now()->addHours(1); // 1 heure au lieu de 1 semaine
    $token->token->save();
    return $token;
}
}
